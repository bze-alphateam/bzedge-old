...
#if defined(NDEBUG)
# error "BZEdge cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "BZEdge Signed Message:\n";
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    else if ( nHeight < 4875001 ) nSubsidy = (1 * COIN);
    else {
        nSubsidy = 0 * COIN;
    }
    return nSubsidy;
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    // Check timestamp (original Zcash)
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In BZEdge this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see BZEdge protocol spec
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION || pfrom->nVersion > MAX_PEER_PROTO_VERSION)
        {
            // disconnect from peers older than this proto version
            LogPrintf("peer=%d using obsolete version %i; disconnecting\n", pfrom->id, pfrom->nVersion);
            pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
                               strprintf("Version must be between %d and %d", MIN_PEER_PROTO_VERSION, MAX_PEER_PROTO_VERSION));
            pfrom->fDisconnect = true;
            // active defense: should ban unusual proto peer
        ...
        if (pfrom->nVersion < params.vUpgrades[currentEpoch].nProtocolVersion)
        {
            ...
            pfrom->fDisconnect = true;
            // active defense: should ban uncompatible proto peer
            LogPrintf("ACTIVE DEFENSE: ban %s A.S.A.P.\n", pfrom->addr.ToString());
			CNode::Ban(pfrom->addr);
            return false;
        }
